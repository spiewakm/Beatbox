...
* beatbox.py: glowna aplikacja, która wykorzystuje ponadto poniższe moduły:
...
Każdy folder składa się:
	...
	* sampleXY.wav/txt: sample w formacie .wav lub .txt 
	(definicja instrumentu, kazdy plik sampleXY.txt powinien zawierać informacje na temat: "id" instrumentu, 
	"fun" funkcji z której generujemy nutki dla wybranego instrumentu, "duration" czas trwania grania nutki, 
	"vol" glosnosc nutek generowanych z wybranego instrumentu, "attack" oraz "decay").


Utworzone demo można znaleźć w katalogu /var/tmp/nazwa_utworu, gdzie nazwa jest 
taka sama jak nazwa katalogu. Każde demo jest zapisywane w formacie .wav.


Ponadto każdy powyższy folder znajduje się również w spakowanej wersji .zip. 
